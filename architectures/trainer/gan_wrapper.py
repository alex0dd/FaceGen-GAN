...
class GAN_Wrapper(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        zeros_labels = tf.zeros((batch_size, 1))
        ...
        labels = tf.concat(
            [tf.ones((batch_size, 1)), zeros_labels], axis=0
        )
        ...
        misleading_labels = zeros_labels

        # Train the generator (note that we should *not* update the weights
        # of the discriminator)!
        with tf.GradientTape() as tape:
            gen_outputs = self.generator((random_latent_vectors, conditions), training=True)
            predictions = self.discriminator((gen_outputs, conditions), training=False)
